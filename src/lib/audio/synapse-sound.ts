...
const PHASE_GAINS   = [0.018, 0.015, 0.012, 0.020, 0.018] as const;
...
/**
 * Re-tune ambient drone for a given phase index (0-4).
 * Preserves exact ramp times and frequency offsets from v0 lines 1171-1184.
 */
export function setPhase(phaseIdx: 0 | 1 | 2 | 3 | 4): void {
  if (!_ambientNodes || _ambientPhase === phaseIdx) return;
  _ambientPhase = phaseIdx;

  const ctx = getAudioContext();
  if (!ctx) return;

  const t    = ctx.currentTime;
  const freq = PHASE_FREQS[phaseIdx];
  const filt = PHASE_FILTERS[phaseIdx];
  const gain = PHASE_GAINS[phaseIdx];
