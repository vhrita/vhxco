...
async function navigateToPhase(page, phaseIdx) {
  ...
  const success = await page.evaluate((idx) => {
    if (
      window.__phaseStore &&
      typeof window.__phaseStore.goToPhase === "function"
    ) {
...
async function runViewport(browser, width, consoleErrorsMap, summary) {
  ...
  try {
    ...
    // Navigate — ?qa=1 suppresses CustomCursor (eliminates dot/ring artifact in PNGs)
    await page.goto(`${BASE_URL}?qa=1`, {
...
async function main() {
  ...
  await fs.writeFile(
    summaryPath,
    JSON.stringify(
      { timestamp: TIMESTAMP, outDir: OUT_DIR, shots: summary },
      null,
      2,
    ),
  );
